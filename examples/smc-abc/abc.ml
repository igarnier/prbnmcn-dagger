...
let abc_resampling prior_density :
 ...
 fun ~target_size (module P) resampling_state rng_state ->
  ...
  let vec_theta = try Vec.create () with _ -> assert false in
  let vec_w = try Vec.create () with _ -> assert false in
  for _i = 0 to target_size - 1 do
    let p = sampler rng_state in
...
let rec abc_loop dist likelihood y epss theta tau_sq next_score =
  ...
  | [] -> return theta
  | eps :: epss ->
      let prior = sample (Dist.gaussian ~mean:theta ~std:tau_sq) in
...
let prior = Smc.sample (Dist.range { min = -10.; max = 10. })
...
let likelihood theta =
  let open Smc in
  sample
  @@ Dist.mixture [| 0.5; 0.5 |] (function
         | 0 -> Dist.gaussian ~mean:theta ~std:1.0
         | 1 -> Dist.gaussian ~mean:theta ~std:0.01
         | _ -> assert false)
...
let result =
  Smc.run
    ...
    model
    rng_state
  |> Seq.take 20 |> Seq.iteri plot_hist
