...
module Kalman_cps = struct
  ...
  module Smc = Smc_inference.Make (struct
  ...
  let step_model : m:mass -> dt:float -> mass t =
   fun ~m ~dt ->
    let* xwind =
      sample (Stats_dist.gaussian ~mean:0.0 ~std:Dynamics.wind_std)
    in
    let* ywind =
      sample (Stats_dist.gaussian ~mean:0.0 ~std:Dynamics.wind_std)
    in
  ...
  let filtered =
    ...
    let pop =
      ...
      run
        ...
        ()
        ~npart:nparticles
        (model 0 Dynamics.init 0.0 traj)
        rng_state
...
(* module Kalman_eff = struct
 *   open Dagger
 *   open Smc_inference_eff
 *
 *   let step_model : m:mass -> dt:float -> mass =
 *    fun ~m ~dt ->
 *     let xwind = sample (Stats_dist.gaussian ~mean:0.0 ~std:Dynamics.wind_std) in
 *     let ywind = sample (Stats_dist.gaussian ~mean:0.0 ~std:Dynamics.wind_std) in
 *     let xvel = m.vel.x +. (xwind *. dt) in
 *     let yvel = m.vel.y +. ((ywind +. Dynamics.gravity) *. dt) in
 *     let xpos = m.pos.x +. xvel in
 *     let ypos = m.pos.y +. yvel in
 ...
 *     log_score score
 *
 *   let rec model record_particle i estimate t traj =
 *     if i = Array.length traj then ()
 *     else
 *       let (m, t') = traj.(i) in
 *       (\*
 *        predict
 ...
 *        Recurse
 *        *\)
 *       record_particle i estimate ;
 *       model record_particle (i + 1) next_estimate t' traj
 *
 *   let filtered =
 *     let rng_state = Random.State.make [| 0xDEAD; 0xBEEF |] in
 *     let result = Array.make (Array.length Dynamics.noisy_trajectory) [] in
 *     let record_particle i m = result.(i) <- m.pos :: result.(i) in
 *     (\* There's also an interruptible runner that can be used for on-line
 *        filtering *\)
 *     let t0 = Unix.gettimeofday () in
 *     let _pop =
 *       Dagger.Smc_inference_eff.run
 *         nparticles
 *         (fun () ->
 *           model record_particle 0 Dynamics.init 0.0 Dynamics.noisy_trajectory)
 *         rng_state
 *     in
 *     let t1 = Unix.gettimeofday () in
 *     Format.printf "inference time %f@." (t1 -. t0) ;
...
let plot title (filtered : unit Kalman_cps.Smc.population Seq.t) =
  ...
  let spread =
    ...
    |> Seq.map (fun (pos, mean) ->
           ...
           Plot.tup_r2 (median_spread, median_spread))
    |> Data.of_seq
  in
...
let () =
  plot "cps" Kalman_cps.filtered |> Plot.(run ~target:(qt ()) exec_detach)

(* let () = plot "eff" Kalman_eff.filtered |> Plot.(run ~target:(qt ()) exec) *)
