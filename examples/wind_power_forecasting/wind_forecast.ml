...
module Model = struct
  ...
  module Data = struct
    ...
    let power_ticks = Seq.zip monotonic (Float.Array.to_seq coarsened_power)
  ...
  let rec model (state : State.t) record_particle data =
    ...
    | Seq.Cons ((index, observed_power), rest) ->
        ...
        let* () =
          ...
          score_noyield @@ Stats.Pdfs.gaussian ~mean:0.0 ~std:15_000. dist
        in
        let* () =
        ...
        in
        let* () = yield in
        (* Record the resampled population *)
  ...
  let filtered =
    ...
    let record_particle ~state ~at_tick = Hashtbl.add table at_tick state in
  ...
  let cumulative_forecast_by_integration (filtered : (int * State.t) Seq_ops.t)
      =
  ...
  let cumulative_forecast_by_extrapolation
      (filtered : (int * State.t) Seq_ops.t) =
    let filt = Hashtbl.of_seq filtered in
    Seq.map
      (fun (tick, cumu) ->
        let forecast_tick = tick + forecast_span in
