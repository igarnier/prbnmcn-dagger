...
struct
  include Stats.Pdfs
  let poisson_ln ~lambda ~k = Log_space.unsafe_cast (poisson_ln ~lambda ~k)
  let gaussian_ln ~mean ~std x = Log_space.unsafe_cast (gaussian_ln ~mean ~std x)
  let exponential_ln ~rate x = Log_space.unsafe_cast (exponential_ln ~rate x)
  let geometric_ln ~p x = Log_space.unsafe_cast (geometric_ln ~p x)
  let uniform_ln range x = Log_space.unsafe_cast (uniform_ln range x)
  let binomial_ln ~p ~n ~k = Log_space.unsafe_cast (binomial_ln ~p ~n ~k)
  let gamma_ln ~shape ~scale x = Log_space.unsafe_cast (gamma_ln ~shape ~scale x)
end
open Dagger.Dist
