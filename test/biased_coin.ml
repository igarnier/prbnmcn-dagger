...
let stats_weighted arr =
  let (mass, total) =
    List.fold_left
      (fun (mass, total) (b, w) ->
        let w = Log_space.to_float w in
        if b then (mass +. w, total +. w) else (mass, total +. w))
      (0.0, 0.0)
      arr
  in
  let p = mass /. total in
...
let smc =
  ...
  let t0 = Unix.gettimeofday () in
  let res = Non_interruptible.run 10_000 model rng_state in
  let t1 = Unix.gettimeofday () in
  Format.printf "smc: data generated in %f seconds@." (t1 -. t0) ;
  abs_float (stats_weighted res -. (2. /. 3.)) <. 0.01
