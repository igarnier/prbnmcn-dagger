...
module type S = sig
  ...
  (** [run ~nthreads resampling resampling_state ~npart model rng] returns a lazy sequence of populations. The initial
      population has size [npart]. [resampling] corresponds to the resampling strategy. [resampling_state]
      is the initial resampling state. The optional argument [nthreads] corresponds to the number of threads to
      be used when performing inference. Defaults to 1 (no parallelism). Maximum recommended value is
      ...
      @raise Invalid_argument if [nthreads < 1]
  *)
  val run :
    ?nthreads:int ->
    resampling_strategy ->
...
module Make (Resampling_types : Resampling_types_S) () :
  S
    with type particle_output = Resampling_types.particle_output
     ...
     and type 'a dist = Random.State.t -> 'a
