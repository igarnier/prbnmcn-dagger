...
module Make (Resampling_types : Resampling_types_S) () = struct
    ...
    and 'a particle_buffer =
      { mutable suspended : 'a suspended_particle Vector.t;
        ...
        mutable max_score : Log_space.t
            (** The maximum score of all particles pushed in [suspended] or
                [terminated]. *)
      }
  ...
  module Syntax = struct
    ...
    type 'a Cps_monad.effect_ +=
      | Dist of (Random.State.t -> 'a)
      | Score of 'a * Log_space.t
      | Yield : particle_output -> resampling_state Cps_monad.effect_
      | Fork : int -> unit Cps_monad.effect_
      | Get_score : Log_space.t Cps_monad.effect_
      | Set_score : Log_space.t -> unit Cps_monad.effect_
     ...
     fun m f ~handler ->
      { cont =
          (fun k w pop ->
            (m ~handler).cont
              (fun m w pop ->
                let effect_ = Score (m, f m) in
                handler.handler effect_ k w pop)
              w
              pop)
      }
  ...
  module Runner = struct
    ...
    let perform_resampling : type a.
        resampling_strategy -> RNG.t -> a Population_state.t -> unit =
    ...
    and resume_importance_sampling : type a.
        resampling_strategy -> RNG.t -> a Population_state.t -> a =
     fun resampling rng_state pop ->
