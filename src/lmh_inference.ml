...
module Table = struct
  ...
  let handler : RNG.t -> unit Syntax.handler =
    ...
    fun rng_state ->
      { handler =
          (fun (type a) (dist : a Cps_monad.effect_) k () trace ->
            match dist with
...
module MCMC_stats = struct
  ...
  let accept () =
    if Int.equal !total !window then accepted := Int.min (!accepted + 1) !total
    else (
      incr accepted ;
      incr total)

  let reject () =
    if Int.equal !total !window then accepted := Int.max (!accepted - 1) 0
    else incr total

  let set_window_size wsize =
    if wsize <= 0 then invalid_arg "Counter.set_window: window must be positive" ;
    window := wsize
